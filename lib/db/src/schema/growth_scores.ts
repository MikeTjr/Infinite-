...
export const growthScoresTable = pgTable("growth_scores", {
  ...
  coupleId: uuid("couple_id").notNull().unique().references(() => couplesTable.id, { onDelete: "cascade" }),
  total: integer("total").notNull().default(0),
  streak: integer("streak").notNull().default(0),
  lastPlayed: timestamp("last_played"),
  historyJson: jsonb("history_json").$type<Array<{ date: string; score: number; delta: number }>>().notNull().default([]),
  momentum: text("momentum", { enum: ["rising", "steady", "cooling"] }).notNull().default("steady"),
});
